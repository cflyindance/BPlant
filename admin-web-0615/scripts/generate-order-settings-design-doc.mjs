...
const reasons = {
  ...
  "order-surcharge":
    "加收预设（447）、合单加收重算（149）、线上服务费清除（161）；对标 Toast「加收」。",
  "order-settlement":
...
/** seq → groupKey（订单中心 catalog 45 条；v1.9 移除 kitchen-send，送厨迁前厅） */
const assignMap = {
...
push(
  ...
  "",
  "> 文档版本：v1.20（已确认）  ",
  "> 数据范围：订单设置 catalog **26** 条（加收与结算拆为加收 / 金额结算两组）  ",
...
push(
  ...
  "- **126** 为 POS 默认单型；C 端/Kiosk 订单类型见外卖来取 `order-type-pickup`（487–491）。",
