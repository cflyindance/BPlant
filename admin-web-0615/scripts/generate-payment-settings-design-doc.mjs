...
const reasons = {
  ...
  "cds-checkout-ux":
    "客显屏结账步骤：小费/刷卡顺序（9）、小费页（463）、签名（8/464）、自选小票（465）；与扫码端分轨，避免混配。",
  "guest-self-checkout-ux":
    "食客自助结账屏（Kiosk/扫码）：小费流程与展示（492–496）、签名（497）、短信收据（501）；**9↔495** 为客显/扫码端平行项，宜同步默认。",
  "paypad-checkout":
    "Paypad 终端结账页小费/签名/小票；与客显、扫码端分轨，便于按 **终端类型** 配置。",
};
...
push(
  ...
  "### 1.2 设计目标",
  "",
  "- 二级导航收敛为 **8 组**，覆盖 52 条，符合「能付什么 → 怎么算税/小费 → 日结 → 卡费 → **客显怎么付 → 扫码端怎么付** → Paypad」",
...
push(
  ...
  "| **v1.3** | 原 `guest-checkout-ux`（12 条）拆为 **`cds-checkout-ux`**（5）+ **`guest-self-checkout-ux`**（7）；组内顺序按「流程→小费→签名→收据」 |",
  "",
...
for (let i = 0; i < order.length; i++) {
  ...
  if (k === "cds-checkout-ux") {
    push(
      "",
      "**组内顺序**：流程（9）→ 小费（463）→ 签名（8、464）→ 小票（465）。**8** 为刷卡后签名行为，**464** 为签名页显隐，实施时需约定不冲突。",
      "",
    );
  }
...
push(
  ...
  "| 客显·结账交互 | 客显屏 8/9、服务 463–465（小费/签名/小票） |",
  "| 扫码端·结账交互 | 服务设置小费/收据（492–501，食客自助结账屏） |",
