...
function renderPlatformPresetChangelogRows(
  ...
  if (entries.length === 0) {
    const cols = showProductLine ? 5 : 4;
    return `<tr><td colspan="${cols}" class="px-4 py-10 text-center text-sm text-muted-foreground">暂无变更记录</td></tr>`;
  }

  return entries
    .map(
      (e) => `<tr class="border-t border-border hover:bg-muted/30">
        <td class="px-4 py-3 whitespace-nowrap text-muted-foreground">${escapeHtml(formatPlatformPresetChangelogTime(e.at))}</td>
        ${
          showProductLine
            ? `<td class="px-4 py-3 whitespace-nowrap text-card-foreground">${escapeHtml(productLineLabel(e.productLineId))}</td>`
            : ""
        }
        <td class="px-4 py-3 whitespace-nowrap font-mono text-xs text-card-foreground">v${e.version}</td>
        <td class="px-4 py-3 text-card-foreground">${escapeHtml(e.actor)}</td>
        <td class="px-4 py-3 text-card-foreground">${escapeHtml(e.summary)}</td>
      </tr>`,
    )
    .join("");
}
  ...
  return `
    ...
    >
      <button
        ...
        aria-label="关闭"
      ></button>
      <div class="relative z-[1] flex max-h-[min(88dvh,32rem)] w-full max-w-3xl min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-2xl animate-fade-in">
        <div class="flex shrink-0 items-center justify-between gap-3 border-b border-border px-4 py-3">
          <div class="min-w-0">
            ...
            <p class="mt-0.5 text-xs text-muted-foreground">共 ${entries.length} 条记录，按时间倒序</p>
          </div>
          <button
            type="button"
        ...
        <div class="min-h-0 flex-1 overflow-auto">
          <table class="w-full min-w-[36rem] text-sm">
            <thead class="sticky top-0 z-[1] bg-muted/80 text-left text-xs text-muted-foreground backdrop-blur">
              <tr>
                <th class="px-4 py-2.5 font-medium">时间</th>
                ${showProductLine ? `<th class="px-4 py-2.5 font-medium">产线</th>` : ""}
                <th class="px-4 py-2.5 font-medium">版本</th>
                <th class="px-4 py-2.5 font-medium">操作人</th>
                <th class="px-4 py-2.5 font-medium">说明</th>
              </tr>
            </thead>
            <tbody>${renderPlatformPresetChangelogRows(entries, showProductLine)}</tbody>
          </table>
        </div>
